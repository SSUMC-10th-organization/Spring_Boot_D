package com.example.umc10thmission4.global.security.util;
...
public class JwtUtil {
    ...
    public SocialProvider getSocialProvider(String token) {
        try {
            return SocialProvider.valueOf(
                    getClaims(token).getPayload().get("social_provider").toString().toUpperCase()
            );
        } catch (JwtException e) {
            return null;
        }
    ...
    private String createToken(AuthMember member, Duration expiration) {
        ...
        return Jwts.builder()
                ...
                .claim("social_provider", member.getMember().getSocialProvider())  // 소셜 타입 추가
